-- mergeability_unknown is a failed gate recheck, not a conflicting merge.
-- Keep its failure_review successor tied to the durable unknown observation.
...
WHEN json_extract(NEW.binding_json,'$.arm')='failure_review_attempt'
 ...
 AND NOT EXISTS (
   ...
   JOIN gate_input_snapshots s ON s.id=e.snapshot_id
   WHERE i.id=NEW.interrupt_id
     AND json_extract(NEW.binding_json,'$.change_id')=json_extract(s.canonical_json,'$.change.id')
