...
- [x] 逐项实现 PRD §7.1 `/sift approve | reject | retry | hold | ask` 与审批标签的 reason-specific 确定性效果；不在当前 options 内一律拒绝（[#789 Sol PASS](reviews/2026-07-30-m5-command-reason-specific-effects-789-rereview-pi-deepseek-v4-pro.md) / #790：`guardrail_violation`/`code_review` approve 写 `command_effects` 并入队 `gate_re_evaluation`；`merge_conflict`/`failure_review(gate_recheck)` retry 同路径；`failure_review(new_attempt)`/`agent_blocked` retry 经 terminalize+spawn；`design_approval` approve 与通用 reject/hold/ask 保持 #739；optionAllowed 拒绝非 options；`gate_re_evaluation` Complete worker 已由 [#807](https://github.com/miaoxiaoyong/sift/issues/807) 接线——ops 可 claim→Complete→terminal（succeeded 无后继 verdict、failed 结果 union、conflict 替换头均已闭合；HITL/rerun/ready-merge 后继发射留后续））
