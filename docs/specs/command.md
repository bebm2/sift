...
`interrupt_command_effect_bindings` is immutable, one-to-one with an Interrupt, and has the closed reason-tagged schema in [`storage.md` §6.4](storage.md#64-command-targeteffect-与-outcome). Attempt `failure_review` is a tagged `failure_review_attempt`: `gate_recheck` carries exact `change_id/head_sha` and null terminal-attempt fields; `new_attempt` carries the bound failed `terminal_attempt_no/terminal_generation` and null Change/head fields. The retry transaction consumes only these frozen fields and never queries the current Change or guesses which attempt to terminalize. Report quota v1 instead requires `report_quota_failure_review(run_id,daily_bucket_start_ms,daily_bucket_end_ms,security_event_id)`; it has no attempt/retry fields and permits only `reject|hold`, with `hold` retaining `Run=running`. Unknown/missing/cross-reason bindings reject. Gate re-evaluation is the persisted `GateReEvaluationOperationV1` closed operation/result union in [`storage.md` §8.1](storage.md#81-outbox_operations), keyed by its frozen source Interrupt and exact head; `CompleteOutboxAttempt` alone owns succeeded/failed/conflict successors. A closed Interrupt can therefore leave `Run=waiting_human` only behind one identified pending re-evaluation or specified successor. Approval/review/exemption facts are immutable `command_effects` rows consumed by the next Gate snapshot; they never overwrite the historical snapshot.
