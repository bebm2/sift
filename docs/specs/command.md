...
`interrupt_command_effect_bindings` is immutable, one-to-one with an Interrupt, and has the closed reason-tagged schema in [`storage.md` §6.4](storage.md#64-command-targeteffect-与-outcome) for the bindings in this table. It is written by the reason owner in the same transaction that emits the Interrupt; unknown/missing/cross-reason binding rejects. Attempt `failure_review` keeps `retry_kind=gate_recheck|new_attempt` with exact-head / new-attempt recipes. Report quota v1 `failure_review` (`attempt_no=null`) instead requires the storage `report_quota_failure_review(run_id,daily_bucket_start_ms,daily_bucket_end_ms,security_event_id)` arm: it has no `retry_kind`, no absent attempt reference, and permits only its canonical `reject|hold` options; its `hold` is the Report no-transition branch and must CAS-keep the Run `running` without entering `waiting_human`. Gate re-evaluation is a persisted deduplicated internal operation keyed by `(interrupt_id, head_sha)` (with `head_sha=NULL` only for effects that do not re-evaluate a head), and records the creating event; it is not a Forge call inside this transaction. Its succeeded, failed and conflict successors are exclusively the frozen `CompleteOutboxAttempt` branches in [`storage.md` §8.1](storage.md#81-outbox_operations) and §12.4; a closed Interrupt can therefore leave `Run=waiting_human` only behind one identified pending re-evaluation or its specified successor. Approval/review/exemption facts are immutable `command_effects` rows consumed by the next Gate snapshot; they never overwrite the historical snapshot.
