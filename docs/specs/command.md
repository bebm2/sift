...
Only `label.name == labels.approved` and `action == added` may request `approve`. Label commands never manufacture a nonce. `EmitInterrupt` (initial issuance) and `AdvanceInterrupt` / `ApplyCommandEvent` (every rotation) are the only writers that make label approval unavailable: in their nonce/version CAS they set `approval_label_cutoff_position=NULL`. Forge then fully scans that target's approval-label stream outside a transaction and invokes the sole cutover writer, [`SetApprovalLabelCutoff`](storage.md#11-受限写端口), with that persisted `(interrupt_id, expected_version, nonce, position)`. Its CAS is the only writer from NULL to a position. Until it commits, or on a platform without the required position capability, label approval fails closed.
...
|  | reject | — | close/responded; `Run→failed(human_reject)` |
|  | hold | Report-only no-transition binding; Run is running and CAS still matches | set Interrupt `dispatch_state=held,held_reason=manual`, rotate nonce/expiry; retain `Run=running` and its version |
...
|  | reject | — | close/responded; `Run→failed(human_reject)` |
|  | hold | — | common hold |
| attempt `failure_review` | retry | binding declares exactly `gate_recheck` or `new_attempt` | close/responded; respectively enqueue one exact-head Gate re-evaluation, or terminalize bound failed attempt and create next pending attempt/claim/launch |
|  | reject | — | close/responded; `Run→failed(human_reject)` |
|  | hold | — | common hold |
| Report quota v1 `failure_review` | reject | `report_quota_failure_review` binding exactly matches its exhaustion `(run_id,daily_bucket_start_ms,daily_bucket_end_ms,security_event_id)` | close/responded; `Run→failed(human_reject)` |
|  | hold | same binding; Run is running and no-transition CAS still matches | set Interrupt `dispatch_state=held,held_reason=manual`, rotate nonce/expiry; retain `Run=running` and its version |
|  | retry / ask / approve | — | `rejected_option`; this variant has no such canonical option |
...
`interrupt_command_effect_bindings` is immutable, one-to-one with an Interrupt, and has the closed reason-tagged schema in [`storage.md` §6.4](storage.md#64-command-targeteffect-与-outcome). Attempt `failure_review` is a tagged `failure_review_attempt`: `gate_recheck` carries exact `change_id/head_sha` and null terminal-attempt fields; `new_attempt` carries null Change/head and a terminal pair that must **each equal** the binding `(attempt_no,generation)` and FK-match that same Run's `failed` attempt. The retry transaction consumes only these frozen fields and never queries the current Change, selects another failed attempt, or guesses which attempt to terminalize. Report quota v1 instead requires `report_quota_failure_review(run_id,daily_bucket_start_ms,daily_bucket_end_ms,security_event_id)`; it has no attempt/retry/terminal/change fields and permits only `reject|hold`, with `hold` retaining `Run=running`. Unknown/missing/cross-reason bindings reject. Gate re-evaluation is the persisted versioned `GateReEvaluationOperationV1` / `GateReEvaluationResultV1` protocol in [`storage.md` §8.1](storage.md#81-outbox_operations), keyed by its frozen source Interrupt, attempt generation and exact head; `CompleteOutboxAttempt` alone allocates evaluation/event IDs and owns every succeeded/failed/conflict successor. A closed Interrupt can therefore leave `Run=waiting_human` only behind one identified pending re-evaluation or specified successor. Approval/review/exemption facts are immutable `command_effects` rows consumed by the next Gate snapshot; they never overwrite the historical snapshot.
...
The canonical command identity is not an events uniqueness key. The complete stage-key mapping is fixed: a non-retry command has exactly `command:<event_key>:initial` (that one event is both initial and final); a retry request has `command:<event_key>:initial`; its only legal final keys are `command:<event_key>:final:probe-succeeded`, `command:<event_key>:final:probe-failed`, `command:<event_key>:final:fact-wins`, and `command:<event_key>:final:decision-wins`. The first is the successful absence result; the latter three are respectively absence-unconfirmed, fact-wins and decision-wins. No suffix is appended to another stage key, and no other event type may use these keys. Every retry final event references the initial event by `final_for_event_id`; [`command_event_outcomes`](storage.md#64-command-targeteffect-与-outcome) records that initial/final pair through its one-time CAS. A retry replay therefore returns the persisted final outcome at every crash point. The final ack operation key is `command:<event_key>:ack`; its `created_from_event_id` is the final event and its target is the immutable envelope target. Same key with another digest is a contract violation. It is created in the same final transaction, not after an external probe.
