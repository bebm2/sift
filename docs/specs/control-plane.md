...
`exit_code=0|1|2` 与 [`config.md`](config.md) §7 一致；`level=ok|info|warning|error`，checks 按 id 升序。details 是按 check id 绑定的 closed schema，不得成为任意 JSON 逃生口。任何模式下 details/message 均不得包含 Forge auth 原始输出、token、scope、endpoint 或任意多行子进程输出；认证成功只可报告脱敏的已认证事实，命令探活输出至多保留一条有界版本行。`stage_ms` 是本次诊断各阶段的毫秒耗时（非 SLA、不得参与 exit code）；可能出现的 key 固定为 `exec`、`process_group`、`sqlite`、`version`、`outbox`、`hooks`、`policy`、`attempts`。缺少 config 时不产生依赖 config 的阶段 key。
