...
`gate_re_evaluation` 唯一使用 `GateReEvaluationOperationV1` closed payload（`payload_schema_version=1`）：`{source_interrupt_id,source_command_event_id,run_id,attempt_no,generation,change_id,head_sha,gate_input_snapshot_id,gate_input_hash,gate_version,effect_binding_digest,operation_key}`。source/event/run/attempt/generation/change/head/snapshot/hash/version/binding digest 均为冻结身份；`(run_id,attempt_no,generation)` 必须命中 source Interrupt 绑定的 attempt，`operation_key` 精确为 `gate:<source_interrupt_id>:<head_sha>:reeval:1`。

worker 只在事务外提交 `GateReEvaluationResultV1` 的 canonical JSON bytes：`{schema_version:1,kind,payload}`，unknown field、非 canonical bytes 或 digest 不符一律拒绝。`succeeded` payload 是 `{gate_input_json,gate_input_hash,gate_version,verdict_json,verdict_digest}`：Complete 验证 input/hash/version、重算 verdict digest，并在本事务分配 snapshot/evaluation ID；结果不得携 evaluation ID。`failed` 是 `{failure_class,failure_evidence_digest}`：两者是 worker 已验证的 closed failure evidence，Complete 用稳定 event key `gate:<source_interrupt_id>:<head_sha>:reeval:1:failed` 插入 failure event，取得其 ID 后唯一构造 facts `{failure_class,failure_evidence_ref:"sift://event/<event_id>",recommended_action:"retry"}`、其 canonical failure digest 和 attempt generation preimage/key；结果不得携 event ID、event ref 或 failure digest。`conflict` 是 `{replacement_head_sha,replacement_input_json,replacement_input_hash,replacement_facts_digest}`，四字段全非空并通过 Forge/head 绑定校验，不能从错误文本猜 head。三 arm 字段互斥且 required/null closed。这消除了“结果先引用 complete 才创建的 evaluation/event”的循环。

claim/replay 由同一 key 返回既有 operation。`CompleteOutboxAttempt` 是唯一 terminal owner，并在同一 lease CAS 事务中插入 result、事件、source Interrupt close 和下列逐 verdict 后继；每行的 Run CAS、事件和后继全成全败：

| `VerdictV1` | Run / source Interrupt | 唯一后继 |
|---|---|---|
| `failed/change_not_open`、`failed/hard_guardrail` | Run→`failed(gate_verdict)`；source close/responded | 无 |
| `wait_checks/checks_pending` | Run→`running(gate_waiting_checks)`；source close/responded | 无；reconciler 只重观测同一 Change |
| `retry_checks/flaky_retry` | Run→`running`；source close/responded | 一条冻结 head/check/retry-no 的 `rerun_checks` |
| `hitl/checks_timeout`、`hitl/failure_review`、`hitl/mergeability_unknown`、`hitl/input_unknown` | Run 保持/转 `waiting_human`；source close/responded | 一条该 verdict facts 的 `failure_review` Interrupt |
| `hitl/guardrail_violation`、`hitl/code_review`、`hitl/merge_conflict` | Run 保持/转 `waiting_human`；source close/responded | 一条对应 reason、exact head/binding 的 Interrupt |
| `ready/merge` | Run→`running`；source close/responded | 一条 exact-head `merge_change` |
| `ready/no_auto_merge` | Run→`succeeded(gate_passed_no_auto_merge)`；source close/responded | 无 |

`failed` result 不经 VerdictV1：它关闭 source Interrupt/responded，保持 Run `waiting_human`，并原子创建/去重一条 open `failure_review`；其 binding 是刚由 Complete 构造的 `failure_review_attempt(run_id,attempt_no,generation,gate_recheck,change_id,head_sha,NULL,NULL)`。`conflict` 关闭 source Interrupt/responded，Run 保持 `waiting_human`，并创建/去重 `gate:<source_interrupt_id>:<replacement_head_sha>:reeval:1`；replacement head 必须不同于 operation head，故 terminal conflict 不会自环。无 verified replacement head 的 conflict 强制走 failed arm。重放或 lease loss 返回既有 terminal result/successor，不得由 worker 另调 `EmitInterrupt` 或 `RecordGateEvaluation`。claim 前、lease 后外部读取前、读取后 complete 前及 complete 后的 crash vectors 都断言最多一个 terminal event/evaluation/Interrupt/operation；`rerun_checks` 的 claim/reclaim、request-start 与 complete 另按 §8.5 执行，不得套用通用 lease-expiry 重试。
