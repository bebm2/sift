...
- `failure_review_attempt(run_id,attempt_no,generation,retry_kind,change_id,head_sha,terminal_attempt_no,terminal_generation)`。`retry_kind=gate_recheck` 时 `change_id`、`head_sha` 必填，`terminal_attempt_no`/`terminal_generation` 必须为 NULL；`retry_kind=new_attempt` 时 `change_id`/`head_sha` 必须为 NULL，`terminal_attempt_no`/`terminal_generation` 必填且分别等于被终结 attempt 的身份。两种 arm 都要求 `run_id`、attempt/generation 与 Interrupt 绑定的 Run/attempt 相等；`gate_recheck` 的 head 是 exact checked head，不能从当前 Change 查询补齐。
- `report_quota_failure_review(run_id,daily_bucket_start_ms,daily_bucket_end_ms,security_event_id)`。其 `(run_id,daily_bucket_start_ms)` 必须命中 `report_quota_exhaustions`，end/security event 必须逐列相等；它没有 attempt、generation、retry 或 change/head 字段，只与 `failure_review` reason 和 `reject|hold` options 合法。

每个 arm 的 required/null 字段、组合 FK（包括 attempt/run、Change/head 和 exhaustion/security-event）及 reason/options 一致性由写端口及 CHECK 共同保证；`binding_digest=SHA-256(canonical_json(binding_json))`。未知 tag、字段缺失、额外字段、跨 reason、错配 FK 或重复 binding 一律回滚。
...
`gate_re_evaluation` 唯一使用 `GateReEvaluationOperationV1` closed payload（`payload_schema_version=1`）：`{source_interrupt_id,source_command_event_id,run_id,change_id,head_sha,gate_input_snapshot_id,gate_input_hash,gate_version,effect_binding_digest,operation_key}`，其中 source/event/change/head/snapshot/hash/version/binding digest 均必填且为冻结身份；`operation_key` 精确为 `gate:<source_interrupt_id>:<head_sha>:reeval:1`。worker 只能在事务外提交同 shape 的 verified result；result union 为 `succeeded{evaluation_id,verdict_digest,verdict_kind,verdict_code}`、`failed{failure_class,failure_digest,failure_event_id}`、`conflict{replacement_head_sha,replacement_snapshot_id,replacement_input_hash,replacement_facts_digest}`。每个 arm 的字段互斥且 required/null closed；`succeeded` 的 evaluation 必须引用该 snapshot/head，`conflict` 的 replacement 字段必须全非空并通过 Forge/head 绑定校验，不能从错误文本猜 head。

claim/replay 由同一 key 返回既有 operation。`CompleteOutboxAttempt` 是唯一 terminal owner，并在同一 lease CAS 事务中执行以下 union：`succeeded` 插入/复用 Gate evaluation，写 succeeded event，关闭 source Interrupt/responded，并按 verdict 的唯一 Gate 映射推进 Run；`failed` 插入 failure event，关闭 source Interrupt/responded，原子创建/去重一个 open `failure_review` Interrupt，其 binding 是 `failure_review_attempt(run_id,attempt_no,generation,gate_recheck,change_id,head_sha,NULL,NULL)`，并保持 Run waiting behind it；`conflict` 插入 conflict event，关闭 source Interrupt/responded，创建/去重 `gate:<source_interrupt_id>:<replacement_head_sha>:reeval:1` successor，且 Run 继续等待该 successor。无 verified replacement head 的 conflict 强制走 failed arm。结果、operation CAS、Run/Interrupt transition、event 与 successor 全成全败；重放或 lease loss 返回既有 terminal result/successor，不得由 worker 另调 `EmitInterrupt` 或 `RecordGateEvaluation`。claim 前崩溃可重试；claim 后外部调用前、调用后但 complete 前、complete 提交后崩溃分别由 lease/replay 收敛，绝不产生第二 successor。`rerun_checks` 的 claim/reclaim、request-start 与 complete 另按 §8.5 执行，不得套用通用 lease-expiry 重试。
